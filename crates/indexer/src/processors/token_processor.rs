...
use diesel::{
    pg::upsert::excluded, result::Error, DecoratableTarget, ExpressionMethods, PgConnection,
};
    ...
    for (start_ind, end_ind) in chunks {
        match execute_with_better_error(
            ...
            diesel::insert_into(schema::current_token_ownerships::table)
                ...
                // .filter_target(last_transaction_version.lt(excluded(last_transaction_version)))
                .do_update()
                .set((
